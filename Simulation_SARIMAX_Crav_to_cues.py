################################################################################
#### Step 1 = After import of CravCues.npy --> reduction of time points  #######
################################################################################

import numpy as np
import pandas as pd

file_path = 'CravCues.npy'
data = np.load(file_path, allow_pickle=True)
Luse, Lcrav, Lcues = data
...
print(f"Nombre total de sujets uniques : {num_subjects}")
time_points_per_subject = df_reduced.groupby('SubjectID')['Time'].count()
print("Nombre de points temporels par SubjectID :")
...
# Définir une graine pour la reproductibilité de la sélection aléatoire
np.random.seed(42) 

# Si le nombre de sujets est inférieur à 4, on utilise tous les sujets disponibles, sinon on en prend 4
num_samples = min(4, len(subject_ids))
sample_subject_ids = np.random.choice(subject_ids, num_samples, replace=False)

# Création de tracés pour les sujets sélectionnés
...
for i, subject_id in enumerate(sample_subject_ids, 1):
    ...
    plt.xticks(range(5))  # Affiche les points de 0 à 4 sans décimales
    plt.grid(True)
